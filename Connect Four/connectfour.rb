...
def drop_chip(selection_parameter, board_parameter)
  ...
  board_parameter.each do | row |
    ...
    if row[selection_parameter - 1] == 0
      counter+=1
    end

  end

  board_parameter[counter - 1][selection_parameter - 1] = 1
  build_board(board_parameter)

end

# method for computer dropping the chip
def computer_drop_chip(board_parameter)
...
def sw_ne_four_diagonal(the_board_parameter)
  ...
  # I dont want to re-check, if I already found 4 diagonally
  if score < 4
    total_rows = 8
    score = 0
    total_cols = 8
    ...
    while remaining_cols > 0 && (score < 4)
      while(row_pos >= 0 && col_pos < total_cols)
        current_value = the_board_parameter[row_pos][col_pos]
        #puts "next at [#{col_pos}, #{row_pos}] the current value is #{current_value} and the score is #{score}"
        if (current_value != 0) && (current_value == prev_val || prev_val == 0)
          score+=current_value
        elsif (current_value != 0)
          score = current_value
...
while continue
  ...
  while my_turn && continue
    ...
    while (selection < 1) || (selection > 8)
      ...
      print "You didn't enter a (1 - 8). Try again."
      selection = gets.to_i

    end

    drop_chip(selection, the_board)
    my_turn = false
  end

  # win check
  # BRING THIS BACK AFTER DIAGONAL ARE SORTED OUT
